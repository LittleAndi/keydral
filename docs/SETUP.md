...
| ----- | ---- | ------- | ----------- |
| `Algorithm` | `string` | `"AES-256-GCM"` | Encryption algorithm. **Only `"AES-256-GCM"` is supported.** Setting any other value causes a startup error (`InvalidOperationException`). |
...
> **Important:** Setting `Algorithm` to any value other than `"AES-256-GCM"` — for example `"AES-128-GCM"` or `"ChaCha20"` — will cause Keydral to fail at startup with a clear `InvalidOperationException`. No other algorithms are implemented or planned.
